...
import { authRegister } from "./auth.action";
import { useNavigate } from "react-router-dom";
import { Modal, Button, Form } from "react-bootstrap";

const SignUp = ({ authRegister, status, message, data }) => {
  ...
  const validateForm = (values) => {
    ...
    // Phone number validation (optional, adjust based on your requirements)
    if (values.phone_number && !/^\d+$/.test(values.phone_number)) {
      errors.phone_number = "Invalid phone number format (digits only).";
    }
    ...
    // Gender can remain as-is (optional field)

    // Password validation (consider using a password library for more robust rules)
    if (!values.password) {
              ...
              <form>
                ...
                <div className="mb-5">
                  <button
                    ...
                    className="w-full cursor-pointer disabled:bg-slate-600 rounded-lg border border-primary bg-primary p-4 text-white transition hover:bg-opacity-90"
                  >
                    Submit
                  </button>
                </div>
...
const SelectOptionOne = ({ isChecked, handleRadioChange }) => {
...
const MessageWindows = ({ show, handleClose, message }) => {
  return (
...
const SignUpComponent = connect((state) => {
  return {
